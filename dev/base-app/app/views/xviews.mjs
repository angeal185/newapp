...
const xviews = {
  build(app_main){
    ...
    let toTop = x('div', {
      class: 'totop ico-up-open hidden',
      onclick: function(){
        window.scroll({
          top: 0,
          left: 0,
  ...
  dashboard(stream, data){
    ...
    lgroup = x('ul', {class: 'todo-list list-group'}),
    item = x('div',
      x('div', {class: 'container-fluid mb-4'},
        x('h2', 'Todo Items'),
        lgroup
      ),
      x('div', {class: 'container-fluid mb-4'},
    ...
    utils.getList(function(err,res){
  ...
  about(stream, data){
    let item = x('div', {class: 'container-fluid text-center'},
      x('img', {class: 'img-fluid', src: data.img}),
      x('p', data.msg)
    );
